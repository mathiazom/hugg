...
public class ArchitectureTests(ITestOutputHelper testOutputHelper)
{
    ...
    private static readonly Assembly InfrastructureAssembly = Assembly.GetAssembly(
        typeof(Infrastructure.DependencyInjection)
    );
    private static readonly Assembly ApplicationAssembly = Assembly.GetAssembly(
    ...
    private void LogIfFailure(TestResult result)
    {
        ...
        testOutputHelper.WriteLine(
            "Types that failed to meet the rule: " + string.Join(", ", result.FailingTypes)
        );
    }
}
